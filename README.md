...
A desk instrument that shows how much of your Claude subscription you have
spent. A [Waveshare ESP32-S3-Touch-AMOLED-2.16][board-docs] renders the two
rolling windows — five-hour session and seven-day — on a 480 × 480 AMOLED,
alongside animated sprites, battery telemetry and a clock. A host daemon reads
the numbers and pushes them over BLE.

Everything is Rust: `esp-hal` and Embassy on the device, Slint for the UI,
[ergot][ergot] over a Nordic UART link for the wire protocol, and Tokio on the
host.

The idea comes from [Clawdmeter][clawdmeter], a C++/Arduino project for the
same board. This is not a port — firmware, protocol, sprites and daemon were
written from scratch, and none of Clawdmeter's artwork is reused, since it is
scraped from copyrighted Anthropic material.

## Current status

Working end to end on hardware: usage is read on the host, published over BLE
NUS, and displayed on the panel.

- `esp-hal` 1.1 with Embassy/`esp-rtos`
- BLE peripheral on `trouble-host` advertising a Nordic UART service
- ergot frames over NUS, one frame per GATT write or notification
- palette-indexed sprite animations, generated by `tools/gen_animations.py`
...
IMU, audio and SD are not wired yet.
...
```sh
git clone https://github.com/okhsunrog/claudial.git
cd claudial
